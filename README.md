# 📊 Delhivery Small World Dashboard

A modern and interactive **Power BI dashboard** designed to analyze business performance across revenue, customers, profit, and returns.

---

## 🚀 Project Overview

This dashboard provides a **comprehensive business intelligence view** of sales and customer data. It helps stakeholders quickly understand:

- Revenue trends 📈  
- Customer behavior 👥  
- Profit insights 💰  
- Return analysis 🔄  

Built with a focus on **clarity, usability, and decision-making**.

---

## 🧩 Features

- 📅 Year Filter (2024 & 2025)
- 📊 Revenue Trend Analysis (Monthly)
- 🧮 Key Metrics Cards:
  - Total Revenue  
  - Total Profit  
  - Total Customers  
  - Return Rate  
- 🌍 Regional Performance Analysis
- 🛍️ Category-wise Revenue & Returns
- 🧾 Top Products & Customers Insights
- 📦 Sub-category Return Breakdown

---

## 📸 Dashboard Preview

### 🏠 Main Overview
<img width="1140" height="640" alt="Screenshot 2026-04-16 133009" src="https://github.com/user-attachments/assets/95867f68-5019-4d0e-a693-e62842599ec7" />


### 📦 Product & Return Insights
<img width="1136" height="639" alt="Screenshot 2026-04-16 133047" src="https://github.com/user-attachments/assets/07522b09-1291-4c50-b75c-8bac3d2ccda8" />


### 👥 Customer Insights
<img width="1137" height="639" alt="Screenshot 2026-04-16 133027" src="https://github.com/user-attachments/assets/3b1f1a36-56f1-4e10-84bb-0020c029c879" />

### 👥 Tooltips Insights
<img width="1309" height="735" alt="Screenshot 2026-04-16 133120" src="https://github.com/user-attachments/assets/31ec33f8-de36-4f43-a68d-6f995faf38b7" />


---

## 🛠️ Tech Stack

- Power BI  
- Data Modeling  
- DAX (Data Analysis Expressions)  
- Data Visualization  


---

## 📌 Key Insights

- Office Supplies generate the highest revenue 🏆  
- South region leads in overall revenue 🌍  
- Return rate is relatively low (~5%) ✔️  
- Binders & Chairs have higher return counts  

---

## 🎯 Use Cases

- Business Performance Tracking  
- Sales Analysis  
- Customer Segmentation  
- Decision Making  

---

## 🤝 Contributing

Feel free to fork this repo and improve the dashboard or add more insights.

---

## 👨‍💻 Author

**MAHESH LOHAR**

- 💼 LinkedIn: linkedin.com/in/mahesh-lohar-557762249 
- 📧 Email: maheshlohar2214@gmail.com  

---
## ⭐ iF you like this project , don't forgot to star the repo!
